...
export default function Subpod({
  updatePodBalance,
  bankPod,
  subPod,
  setSubPods,
  ...
  const subtractMoney = async (event) => {
    ...
    if (newSubBalance < 0) {
      setModalError(
        `You cannot subtract $${subtractAmount}. Subpod will be negative.`
      );
      return;
    }
    updateSubPodBalance(subPod.id, newSubBalance);
  ...
  return (
    <div>
          ...
          <div className="d-flex flex-column justify-content-between">
            ...
            >
              <h4 style={{ marginTop: "8px" }}>
                $
                {subPod.balance.toLocaleString(undefined, {
      ...
      >
        ...
        {modalError.length > 1 ? (
          <div className={`border px-1 py-2 my-2 text-center text-sm`}>
            {modalError}
          </div>
        ) : null}
      </Modal>
