...
export default function Podsdash({ user }) {
  ...
  const [totalNetWorth, setTotalNetWorth] = useState(0); // new state variable for total net worth
  ...
  const calculateTotalNetWorth = () => {
    let total = 0;
    bankPods.forEach((bankPod) => {
      total += bankPod.accountTotal;
    });
  ...
  return recoveryToken ? (
    ...
    <Container className="p-3" fluid style={{ height: "100%" }}>
      <Card style={{ height: "100%", minHeight: "calc(96vh - 61px)" }}>
        ...
        >
          <Card style={{ marginRight: "30px" }} className="p-1 text-center">
            {" "}
            {/* add text-center class to Card component */}
